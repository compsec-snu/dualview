...
export async function runCronIsolatedAgentTurn(params: {
  ...
  // Allow plugins to transform external webhook/email content before the LLM sees it.
  if (isExternalHook) {
    const hookRunner = getGlobalHookRunner();
    if (hookRunner?.hasHooks("transform_webhook_content")) {
      try {
        const hookType = getHookType(baseSessionKey);
        const result = await hookRunner.runTransformWebhookContent(
          {
            message: commandBody,
          ...
          { sessionKey: agentSessionKey, agentId },
        );
        if (result?.message) {
          commandBody = result.message;
        }
      } catch (err) {
        logWarn(`[cron:${params.job.id}] transform_webhook_content hook failed: ${String(err)}`);
      }
    }
  }
