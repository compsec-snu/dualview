...
import { linkDualViewSymbolsForSlack, linkSlackBlockSymbolsForSlack } from "./format.js";
...
export async function editSlackMessage(
  ...
  const linkedBlocks = blocks ? linkSlackBlockSymbolsForSlack(blocks, symbolLinkUrl) : undefined;
  const trimmedContent = content.trim();
  const fallbackText =
    trimmedContent || (linkedBlocks ? buildSlackBlocksFallbackText(linkedBlocks) : " ");
  await client.chat.update({
    channel: channelId,
    ts: messageId,
    text: linkDualViewSymbolsForSlack(fallbackText, symbolLinkUrl),
    ...(linkedBlocks ? { blocks: linkedBlocks } : {}),
  });
}
