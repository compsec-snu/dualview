...
import { resolveDualViewDashboardUrl } from "../gateway/dualview-dashboard-links.js";
...
function buildSlackDashboardSessionKey(channelId: string): string | undefined {
  const normalized = channelId.trim().toLowerCase();
  if (!normalized || !/^[cg][a-z0-9]+$/i.test(channelId)) {
    return undefined;
  }
  return `agent:main:slack:channel:${normalized}`;
}
...
export async function sendMessageSlack(
  ...
  const symbolLinkUrl = resolveDualViewDashboardUrl({
    sessionKey: buildSlackDashboardSessionKey(channelId),
  });
  const applyMessageSendingHook = async (sourceText: string): Promise<string | null> => {
    if (opts.skipMessageSendingHook || !sourceText) {
      return sourceText;
    }
    const hookResult = await runOutboundMessageHook({
      ...
      channel: "slack",
      accountId: account.accountId,
      metadata: { originalTo: to, threadTs: opts.threadTs },
    });
    if (hookResult === null) {
      return null;
    }
    return linkResolvedDataflowSymbolsForSlack({
      sourceText,
      resolvedText: hookResult.content,
      symbolLinkUrl,
    });
  };
  if (blocks) {
    ...
    const linkedBlocks = linkSlackBlockSymbolsForSlack(blocks, symbolLinkUrl);
    const sourceFallbackText = trimmedMessage || buildSlackBlocksFallbackText(blocks);
    const fallbackText = await applyMessageSendingHook(sourceFallbackText);
    if (fallbackText === null) {
      return { messageId: "cancelled-by-hook", channelId };
    }
    const linkedFallbackText = linkDualViewSymbolsForSlack(fallbackText, symbolLinkUrl);
    const response = await postSlackMessageBestEffort({
      client,
      channelId,
      text: linkedFallbackText,
  ...
  const outboundText = await applyMessageSendingHook(trimmedMessage);
  if (outboundText === null) {
    return { messageId: "cancelled-by-hook", channelId };
  }
  const linkedOutboundText = opts.skipMessageSendingHook
    ? linkDualViewSymbolsForSlack(outboundText, symbolLinkUrl)
    : outboundText;
  const markdownChunks =
    chunkMode === "newline"
      ? chunkMarkdownTextWithMode(linkedOutboundText, chunkLimit, chunkMode)
      : [linkedOutboundText];
  const chunks = markdownChunks.flatMap((markdown) =>
    markdownToSlackMrkdwnChunks(markdown, chunkLimit, { tableMode, symbolLinkUrl }),
  );
  if (!chunks.length && linkedOutboundText) {
    chunks.push(linkedOutboundText);
  }
  const mediaMaxBytes =
