...
function resolveDashboardSessionId(params: {
  cfg: OpenClawConfig;
  agentId: string;
  sessionKey?: string;
}): string | undefined {
  ...
  try {
    ...
    return resolveSessionStoreEntry({ store, sessionKey }).existing?.sessionId;
  } catch {
    return undefined;
  }
}
...
export async function dispatchPreparedSlackMessage(prepared: PreparedSlackMessage) {
  ...
  const dashboardSessionKey = prepared.ctxPayload.SessionKey ?? route.sessionKey;
  const symbolLinkUrl = resolveDualViewDashboardUrl({
    sessionKey: dashboardSessionKey,
    sessionId: resolveDashboardSessionId({
      cfg,
  ...
  const resolveSlackOutboundText = async (
    sourceText: string,
    format: "slack_mrkdwn" | "markdown" = "slack_mrkdwn",
  ): Promise<string | null> => {
  ...
  const deliverWithStreaming = async (payload: ReplyPayload): Promise<void> => {
    ...
    const outboundMarkdown = await resolveSlackOutboundText(payload.text.trim(), "markdown");
    if (outboundMarkdown === null) {
      return;
    }
    const text = markdownToSlackStreamingMarkdown(outboundMarkdown);
  ...
  const { dispatcher, replyOptions, markDispatchIdle } = createReplyDispatcherWithTyping({
    ...
    deliver: async (payload) => {
      ...
      if (canFinalizeViaPreviewEdit) {
        draftStream?.stop();
        try {
          const outboundText = await resolveSlackOutboundText(finalText.trim());
          ...
          await ctx.app.client.chat.update({
            ...
            ts: draftMessageId,
            text: normalizeSlackOutboundText(outboundText, { symbolLinkUrl }),
          });
          return;
        } catch (err) {
          logVerbose(
  ...
  const updateDraftFromPartial = (text?: string) => {
    ...
    if (streamMode === "append") {
      ...
      draftStream.update(linkDualViewSymbolsForSlack(next.rendered, symbolLinkUrl));
    ...
    draftStream.update(linkDualViewSymbolsForSlack(trimmed, symbolLinkUrl));
    hasStreamedMessage = true;
  };
