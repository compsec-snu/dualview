...
import type { PluginHookToolResultEvent } from "../plugins/types.js";
...
export async function runToolResultHook(args: {
  toolName: string;
  toolCallId?: string;
  params: unknown;
  result: unknown;
  ...
  const hookRunner = getGlobalHookRunner();
  if (!hookRunner?.hasHooks("transform_tool_result") && !hookRunner?.hasHooks("tool_result")) {
    return args.result as AgentToolResult<unknown>;
  }
  try {
    const normalizedParams = isPlainObject(args.params) ? args.params : {};
    const event: PluginHookToolResultEvent = {
      toolName: normalizeToolName(args.toolName),
      toolCallId: args.toolCallId,
      params: normalizedParams,
      result: args.result,
    };
    const hookResult = await hookRunner.runTransformToolResult(event, {
      toolName: normalizeToolName(args.toolName),
      toolCallId: args.toolCallId,
    ...
    if (hookResult?.result !== undefined) {
      if (args.ctx?.allowResultModification !== true) {
        log.warn(
          `[security] tool_result result replacement blocked for ${args.toolName}: ` +
            `plugins.allowResultModification is not enabled`,
        );
        return args.result as AgentToolResult<unknown>;
      }
    ...
    log.warn(`tool_result hook failed: tool=${args.toolName} error=${String(err)}`);
  }
  return args.result as AgentToolResult<unknown>;
}
  ...
  const wrappedTool: AnyAgentTool = {
    ...
    execute: async (toolCallId, params, signal, onUpdate) => {
      ...
      try {
        ...
        // Let tool_result intercept error results too.
        const errorMessage = err instanceof Error ? err.message : String(err);
        const errorResult: AgentToolResult<unknown> = {
          content: [{ type: "text", text: `Error: ${errorMessage}` }],
          details: { error: true },
        };
        const hookProcessed = await runToolResultHook({
          toolName,
          toolCallId,
          params: outcome.params,
          result: errorResult,
