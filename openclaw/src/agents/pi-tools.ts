...
function includeRunAllowedTools<TPolicy extends { allow?: string[]; deny?: string[] }>(
  policy: TPolicy | undefined,
  runAllowlist: readonly string[] | undefined,
): TPolicy | undefined {
  if (!policy?.allow || !runAllowlist || runAllowlist.length === 0) {
    return policy;
  }
  return { ...policy, allow: Array.from(new Set([...policy.allow, ...runAllowlist])) };
}
...
export function createOpenClawCodingTools(options?: {
  ...
  sessionId?: string;
  /** Optional per-run allowlist that restricts tools to the listed names. */
  ...
  const runToolPolicy =
    Array.isArray(options?.toolAllowlist) && options.toolAllowlist.length > 0
  ...
  const runToolAllowlist = runToolPolicy?.allow;
  const profilePolicyForRun = includeRunAllowedTools(
    profilePolicyWithAlsoAllow,
    runToolAllowlist,
  );
  const providerProfilePolicyForRun = includeRunAllowedTools(
    providerProfilePolicyWithAlsoAllow,
    runToolAllowlist,
  );
  const globalPolicyForRun = includeRunAllowedTools(globalPolicy, runToolAllowlist);
  const globalProviderPolicyForRun = includeRunAllowedTools(
    globalProviderPolicy,
    runToolAllowlist,
  );
  const agentPolicyForRun = includeRunAllowedTools(agentPolicy, runToolAllowlist);
  const agentProviderPolicyForRun = includeRunAllowedTools(
    agentProviderPolicy,
    runToolAllowlist,
  );
  const groupPolicyForRun = includeRunAllowedTools(groupPolicy, runToolAllowlist);
  ...
  const subagentPolicy =
    isSubagentSessionKey(options?.sessionKey) && options?.sessionKey
      ...
      : undefined;
  const sandboxPolicyForRun = includeRunAllowedTools(sandbox?.tools, runToolAllowlist);
  const subagentPolicyForRun = includeRunAllowedTools(subagentPolicy, runToolAllowlist);
  const allowBackground = isToolAllowedByPolicies("process", [
    profilePolicyForRun,
    providerProfilePolicyForRun,
    globalPolicyForRun,
    globalProviderPolicyForRun,
    agentPolicyForRun,
    agentProviderPolicyForRun,
    groupPolicyForRun,
    sandboxPolicyForRun,
    subagentPolicyForRun,
    runToolPolicy,
  ]);
  ...
  const subagentFiltered = applyToolPolicyPipeline({
    ...
    steps: [
      ...buildDefaultToolPolicyPipelineSteps({
        profilePolicy: profilePolicyForRun,
        profile,
        providerProfilePolicy: providerProfilePolicyForRun,
        providerProfile,
        globalPolicy: globalPolicyForRun,
        globalProviderPolicy: globalProviderPolicyForRun,
        agentPolicy: agentPolicyForRun,
        agentProviderPolicy: agentProviderPolicyForRun,
        groupPolicy: groupPolicyForRun,
        agentId,
      }),
      { policy: sandboxPolicyForRun, label: "sandbox tools.allow" },
      { policy: subagentPolicyForRun, label: "subagent tools.allow" },
      { policy: runToolPolicy, label: "run tools.allow" },
    ],
  });
