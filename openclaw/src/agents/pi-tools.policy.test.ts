...
describe("createOpenClawCodingTools run toolAllowlist", () => {
  it("restricts the final tool set to the per-run allowlist", () => {
    const names = createOpenClawCodingTools({ toolAllowlist: ["read"] }).map((tool) => tool.name);

    expect(names).toEqual(["read"]);
  });

  it("keeps per-run allowlisted tools through earlier agent allowlist filtering", () => {
    const cfg = {
      agents: {
        list: [
          {
