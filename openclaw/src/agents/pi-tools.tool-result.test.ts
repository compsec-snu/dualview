...
type HookRunnerMock = {
  ...
  runTransformToolResult: ReturnType<typeof vi.fn>;
};
...
describe("tool_result hook", () => {
  ...
  it("passes result through when no hook is registered", async () => {
    ...
    expect(hookRunner.runTransformToolResult).not.toHaveBeenCalled();
  });

  it("replaces result when hook returns a new result", async () => {
    const hookRunner = installMockHookRunner();
    hookRunner.hasHooks.mockImplementation(
      (name: string) => name === "before_tool_call" || name === "transform_tool_result",
    );
    hookRunner.runBeforeToolCall.mockResolvedValue(undefined);
    hookRunner.runTransformToolResult.mockResolvedValue({
    ...
    expect(hookRunner.runTransformToolResult).toHaveBeenCalledWith(
