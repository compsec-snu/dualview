...
/**
 * Run the `message_sending` plugin hook against a single payload's text
 * before it's formatted for CLI / JSON output. Mirrors the hook invocation
 * that `deliverOutboundPayloads` already performs for external channel
 * delivery — without this, plugins like ADFI that resolve symbols in
 * outbound text only fired for network-facing sends, and CLI users saw
 * raw `$_ADFI_SYM_*` tokens in their agent response. See openclaw#164.
 *
 * The hook is idempotent (ADFI's handler is a no-op when no symbols are
 * present), so even when delivery DOES happen the inner invocation in
 * `deliverOutboundPayloads` stays harmless.
 */
async function resolveOutboundPayloadText<T extends { text: string }>(
  payload: T,
  params: { to: string; channel: string; accountId?: string },
): Promise<T> {
  if (!payload.text) {
    return payload;
  }
  const result = await runOutboundMessageHook({
    to: params.to,
  ...
  if (result.content === payload.text) {
    return payload;
  }
  return { ...payload, text: result.content };
}

async function resolveOutboundPayloadsText<T extends { text: string }>(
  payloads: readonly T[],
  params: { to: string; channel: string; accountId?: string },
): Promise<T[]> {
  const out: T[] = [];
  for (const p of payloads) {
    out.push(await resolveOutboundPayloadText(p, params));
  }
  return out;
}
...
export async function deliverAgentCommandResult(params: {
  ...
  // Resolve symbols (and any other outbound plugin transforms) in the
  // payload text before either the CLI log or JSON envelope sees it.
  // `deliverOutboundPayloads` runs the same hook per-payload on its own
  // path, so external delivery ends up hook-resolved regardless; this
  // block exists so the `deliver=false` paths (CLI log, JSON output) get
  // the same treatment. See #164.
  const hookTarget = {
    to: effectiveSessionKey ?? opts.to ?? opts.replyTo ?? "cli",
    channel: deliveryChannel ?? "cli",
    accountId: resolvedAccountId,
  };
  const normalizedPayloads = await resolveOutboundPayloadsText<OutboundPayloadJson>(
    normalizeOutboundPayloadsForJson(payloads ?? []),
    hookTarget,
  );
  if (opts.json) {
    runtime.log(
  ...
  const deliveryPayloads = await resolveOutboundPayloadsText<NormalizedOutboundPayload>(
    normalizeOutboundPayloads(payloads),
    hookTarget,
  );
