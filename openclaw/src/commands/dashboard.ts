...
import { resolveDashboardHttpUrl } from "../gateway/control-ui-links.js";
...
export async function dashboardCommand(
  ...
  const dashboardHttpUrl = resolveDashboardHttpUrl(cfg);
  const dashboardUrl = includeTokenInUrl
    ? `${dashboardHttpUrl}#token=${encodeURIComponent(token)}`
    : dashboardHttpUrl;
