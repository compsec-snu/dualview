...
vi.mock("../infra/tailnet.js", () => ({
  pickPrimaryTailnetIPv4: () => "100.64.0.2",
}));
...
describe("dashboardCommand bind selection", () => {
  ...
  it.each([
    {
      label: "maps lan bind to loopback",
      snapshot: { bind: "lan" as const },
      expectedUrl: "http://127.0.0.1:18789/#token=abc123",
    },
    {
      label: "defaults unset bind to loopback",
      snapshot: undefined,
      expectedUrl: "http://127.0.0.1:18789/#token=abc123",
    },
  ])("$label for dashboard URLs", async ({ snapshot, expectedUrl }) => {
  ...
  it("preserves custom bind mode", async () => {
    ...
    expect(mocks.copyToClipboard).toHaveBeenCalledWith("http://10.0.0.5:18789/#token=abc123");
  });
  ...
  it("preserves tailnet bind mode", async () => {
    ...
    expect(mocks.copyToClipboard).toHaveBeenCalledWith("http://100.64.0.2:18789/#token=abc123");
  });
});
