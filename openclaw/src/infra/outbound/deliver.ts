...
import { linkResolvedDataflowSymbolsForSlack } from "../../slack/format.js";
    ...
    const content =
      params.channel === "slack"
        ? linkResolvedDataflowSymbolsForSlack({
            sourceText: params.payloadSummary.text,
            resolvedText: sendingResult.content,
            symbolLinkUrl: resolveDualViewDashboardUrl({
              sessionKey: params.sessionKey,
              sessionId: params.sessionId,
            }),
          })
...
async function deliverOutboundPayloadsCore(
  ...
  const deliverySessionKey = params.mirror?.sessionKey ?? params.session?.key;
  const deliverySessionId = resolveDashboardSessionId({
    cfg,
  ...
  const sessionKeyForInternalHooks = deliverySessionKey;
