...
describe("deliverOutboundPayloads", () => {
  ...
  it("does not link raw content resolved from DualView symbols for Slack after message_sending", async () => {
    ...
    hookMocks.runner.runMessageSending.mockResolvedValue({ content: "Result: Example title" });
    ...
    await withEnvAsync({ DUALVIEW_DASHBOARD_URL: "http://127.0.0.1:3456/bot/" }, async () => {
      await deliverOutboundPayloads({
        ...
        channel: "slack",
        to: "C123",
        payloads: [{ text: "Result: $_DUALVIEW_SYM_web_fetch[a1b2].title" }],
      });
    });

    expect(sendText).toHaveBeenCalledWith(
      expect.objectContaining({
        text: "Result: <http://127.0.0.1:3456/bot/|Example title>",
      }),
    );
  });
