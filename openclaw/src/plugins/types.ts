...
export type PluginHookName =
  ...
  | "tool_result" // @deprecated — use "transform_tool_result"
  | "tool_result_persist"
...
// tool_result hook
export type PluginHookToolResultEvent = {
  toolName: string;
  toolCallId?: string;
  params: Record<string, unknown>;
  result: unknown;
...
export type PluginHookToolResultResult = {
...
// transform_webhook_content hook
// Fires only for external hook sessions (webhooks, Gmail PubSub, etc.) in the
// cron isolated agent runner, before the prompt is sent to the LLM.
// Allows plugins to transform or replace the webhook content (e.g. symbolize
// untrusted data for data-flow integrity tracking).
export type PluginHookTransformWebhookContentEvent = {
  /** The fully-built prompt (after OpenClaw security wrapping). */
  message: string;
  /** Raw external content before OpenClaw wrapping. */
  rawContent: string;
  /** External content source type (e.g. "webhook", "email"). */
  source: string;
  /** Cron job name (e.g. "scholar-alert"). */
  jobName: string;
  /** Cron job ID. */
  jobId: string;
};

export type PluginHookTransformWebhookContentResult = {
  /** If set, replaces the prompt that the LLM sees for this turn. */
  message?: string;
};
...
export type PluginHookHandlerMap = {
  ...
  transform_tool_result: (
    ...
    ctx: PluginHookToolContext,
  ) => Promise<PluginHookToolResultResult | void> | PluginHookToolResultResult | void;
  /** @deprecated Use transform_tool_result */
  tool_result: (
    event: PluginHookToolResultEvent,
    ctx: PluginHookToolContext,
  ) => Promise<PluginHookToolResultResult | void> | PluginHookToolResultResult | void;
  ...
  transform_webhook_content: (
    ...
    | Promise<PluginHookTransformWebhookContentResult | void>
