...
import type {
  ...
  PluginHookToolResultEvent,
  PluginHookToolResultResult,
  ...
  PluginHookTransformWebhookContentEvent,
  PluginHookTransformWebhookContentResult,
} from "./types.js";
...
export function createHookRunner(registry: PluginRegistry, options: HookRunnerOptions = {}) {
  ...
  const mergeToolResult = (
    acc: PluginHookToolResultResult | undefined,
    next: PluginHookToolResultResult,
  ): PluginHookToolResultResult => ({
    result: next.result !== undefined ? next.result : acc?.result,
  });
   ...
   *
   * Collects handlers registered under both "transform_tool_result" and the
   * deprecated "tool_result" name, sorted by priority.
   */
  async function runTransformToolResult(
    event: PluginHookToolResultEvent,
    ctx: PluginHookToolContext,
  ): Promise<PluginHookToolResultResult | undefined> {
    // Collect from both the new and deprecated hook names
    const hooks = [
      ...getHooksForName(registry, "transform_tool_result"),
      ...getHooksForName(registry, "tool_result"),
    ].toSorted((a, b) => (b.priority ?? 0) - (a.priority ?? 0));
    ...
    logger?.debug?.(`[hooks] running transform_tool_result (${hooks.length} handlers, sequential)`);

    let result: PluginHookToolResultResult | undefined;
    for (const hook of hooks) {
      try {
        const handlerResult = await (
          hook.handler as (
            event: PluginHookToolResultEvent,
            ctx: PluginHookToolContext,
          ) => Promise<PluginHookToolResultResult>
        )(event, ctx);
        if (handlerResult !== undefined && handlerResult !== null) {
          result = result !== undefined ? mergeToolResult(result, handlerResult) : handlerResult;
        }
      } catch (err) {
        handleHookError({
          hookName: "transform_tool_result",
  ...
  /** @deprecated Use runTransformToolResult */
  const runToolResult = runTransformToolResult;
  ...
  const mergeTransformWebhookContent = (
    acc: PluginHookTransformWebhookContentResult | undefined,
    next: PluginHookTransformWebhookContentResult,
  ): PluginHookTransformWebhookContentResult => ({
    message: next.message ?? acc?.message,
  });

  /**
   * Run transform_webhook_content hook.
   * Fires only for external hook sessions (webhooks, Gmail, etc.) before the
   * prompt is sent to the LLM. Allows plugins to replace the prompt content.
   */
  async function runTransformWebhookContent(
    event: PluginHookTransformWebhookContentEvent,
    ctx: PluginHookAgentContext,
  ): Promise<PluginHookTransformWebhookContentResult | undefined> {
    return runModifyingHook<"transform_webhook_content", PluginHookTransformWebhookContentResult>(
      "transform_webhook_content",
      event,
      ctx,
      mergeTransformWebhookContent,
    );
  }
  ...
  return {
    ...
    runToolResult, // deprecated alias
    ...
    runSubagentSpawned,
    runSubagentEnded,
    // Webhook content transform
    runTransformWebhookContent,
    // Gateway hooks
    runGatewayStart,
    runGatewayStop,
    // Utility
